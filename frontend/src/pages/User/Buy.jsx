...
export default function Buy() {
    const { id } = useParams() // Get crypto ID from route params
    ...
    // Load crypto data and wallet on mount
    useEffect(() => {
        const headers = {
        ...
        Promise.all([
            ...
            .then(([cryptos, walletData]) => {
                ...
                setCrypto(found) // Save crypto info
                setWallet(walletData) // Save wallet info
            })
            .catch((err) => setMessage({ type: 'error', text: err.message }))
            .finally(() => setLoading(false))
    }, [id])

    // Calculate total cost and check if wallet balance is sufficient
    const totalCost =
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setMessage({
                    type: 'success',
                    text: `Successfully bought ${quantity} ${crypto.symbol}!`,
                })
                ...
                <div className="bg-white rounded-2xl shadow-md p-8">
                    ...
                    {quantity > 0 && (
                        ...
                        >
                            ...
                            <span
                                className={`font-bold text-lg ${insufficientBalance ? 'text-red-500' : 'text-blue-500'}`}
                            >
                                {fmt(totalCost)} €
                            </span>
                        </div>
