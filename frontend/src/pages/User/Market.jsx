...
// Helper function to format numbers as strings with decimals
...
export default function Market() {
    // State variables
    const [cryptos, setCryptos] = useState([]) // List of all cryptos
    const [selected, setSelected] = useState(null) // Currently selected crypto
    const [history, setHistory] = useState([]) // Price history for selected crypto
    const [loadingChart, setLoadingChart] = useState(false) // Loading state for chart
    const [loading, setLoading] = useState(true) // Loading state for market data
    const [error, setError] = useState(null) // Error message
    const navigate = useNavigate()

    // Fetch all cryptocurrencies on component mount
    useEffect(() => {
        fetch('https://127.0.0.1:8000/api/market', {
            ...
            .then((data) => {
                setCryptos(data)
                setSelected(data[0]) // Select first crypto by default
            })
    ...
    // Show loading screen while fetching market data
    if (loading)
        return (
    ...
    // Determine color and positivity for selected crypto
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4 md:p-6">
            ...
            <div className="md:hidden flex flex-col gap-4">
                ...
                {/* Horizontal scrollable crypto list */}
                <div>
                            ...
                            {cryptos.map((crypto) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <p
                                            className={`text-xs font-semibold ${pos ? 'text-emerald-500' : 'text-red-500'}`}
                                        >
                                            {pos ? '+' : ''}
                                            {fmt(crypto.change, 2)}%
                                        </p>
...
// Single crypto item for desktop list
...
// Chart panel component for desktop & mobile
function ChartPanel({
    selected,
    history,
    loadingChart,
    ...
    return (
        <div
            className={`bg-white rounded-2xl shadow-sm ring-1 ring-gray-100 p-5 flex flex-col ${
                mobile ? '' : 'flex-1'
            }`}
        >
            {/* Header with crypto info and Buy/Sell buttons */}
