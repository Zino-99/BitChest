...
// Compute total quantity, total cost, average unit cost, current value, and gain for a crypto asset
function computeCryptoStats(crypto) {
    const totalQty = crypto.purchases.reduce((s, p) => s + p.quantity, 0) // Sum of all purchased quantities
    const totalCost = crypto.purchases.reduce(
        (s, p) => s + p.quantity * p.priceAtPurchase,
        0
    ) // Sum of total cost spent
    const avgUnitCost = totalQty > 0 ? totalCost / totalQty : 0 // Average cost per unit
    const currentValue = totalQty * crypto.currentPrice // Current value based on market price
    const gain = currentValue - totalCost // Profit or loss
    return { totalQty, totalCost, avgUnitCost, currentValue, gain }
}
...
// Component to display a single crypto card with expandable purchase history
function CryptoCard({ crypto }) {
    const [open, setOpen] = useState(false) // Toggle for expanding card
    const { totalQty, totalCost, avgUnitCost, currentValue, gain } =
        computeCryptoStats(crypto)
    const positive = gain >= 0 // Determine if gain is positive
    const color = CRYPTO_COLORS[crypto.symbol] ?? '#43698f' // Default color if symbol not found
    const gainPct =
        totalCost > 0 ? ((gain / totalCost) * 100).toFixed(2) : '0.00' // Gain percentage
    const progress = Math.min(
        100,
        Math.max(0, (currentValue / (totalCost * 1.5)) * 100)
    ...
    return (
        ...
        >
            ...
            {/* Expanded section with purchase history */}
            {open && (
                <div className="border-t border-gray-50 bg-gray-50/40 px-6 py-5">
                    <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-4">
                        Purchase History
                    ...
                    <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {[
                            ...
                            <div
                                key={item.label}
                                className="bg-white rounded-xl p-3 ring-1 ring-gray-100"
                            >
        ...
        <div className="min-h-screen bg-gray-50 py-8 px-4">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                    ...
                    {/* Global gain/loss */}
                    <div
                        className={`rounded-2xl p-5 shadow-sm ring-1 flex items-center gap-4 ${
                            totalPositive
                    ...
                    >
                        ...
                        <div>
                            ...
                            <p
                                className={`text-xl font-bold ${totalPositive ? 'text-emerald-600' : 'text-red-500'}`}
                            >
                                {totalPositive ? '+' : ''}
                                {fmt(totalGain)} €
                            </p>
                            <p
                                className={`text-xs mt-0.5 ${totalPositive ? 'text-emerald-400' : 'text-red-400'}`}
                            >
                                {totalPositive ? '+' : ''}
                                {gainPct}% overall
                            </p>
