...
// Format numbers with fixed decimals and commas
function fmt(n, decimals = 2) {
    return Number(n).toLocaleString('en-US', {
        minimumFractionDigits: decimals,
        maximumFractionDigits: decimals,
...
export default function Sell() {
    const { id } = useParams() // Get crypto id from URL
    const navigate = useNavigate()

    // State variables
    const [crypto, setCrypto] = useState(null) // Selected crypto
    const [ownedQty, setOwnedQty] = useState(0) // User-owned quantity
    const [wallet, setWallet] = useState(null) // User wallet data
    const [quantity, setQuantity] = useState('') // Amount user wants to sell
    const [loading, setLoading] = useState(true) // Loading spinner for fetch
    const [submitting, setSubmitting] = useState(false) // Loading spinner for sell
    ...
    // Fetch crypto market and wallet data on mount
    useEffect(() => {
        const headers = {
            'Content-Type': 'application/json',
            'X-User-Id': user?.id,
        ...
        Promise.all([
            fetch('https://127.0.0.1:8000/api/market', { headers }).then((r) =>
                r.json()
            ),
            fetch('https://127.0.0.1:8000/api/wallet', { headers }).then((r) =>
                r.json()
            ),
        ])
            .then(([marketData, walletData]) => {
                ...
                // Calculate owned quantity from wallet
                const cryptoInWallet = walletData.cryptos.find(
                    (c) => c.id === parseInt(id)
                )
                if (cryptoInWallet) {
                    const qty = cryptoInWallet.purchases.reduce(
                        (s, p) => s + p.quantity,
                        0
                    )
                    setOwnedQty(qty)
    ...
    // Calculate total EUR received if selling
    const totalReceived =
        crypto && quantity ? crypto.currentPrice * parseFloat(quantity) : 0
    // Check if user tries to sell more than they own
    const insufficientCrypto = quantity
        ? parseFloat(quantity) > ownedQty
        : false
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setWallet((w) => ({ ...w, euroBalance: data.newBalance })) // Update wallet
                setOwnedQty((prev) => prev - parseFloat(quantity)) // Update owned quantity
                ...
                setMessage({
                    type: 'error',
                    text:
                        data.message === 'Insufficient crypto balance'
                            ? `You only own ${fmt(data.owned, 4)} ${crypto.symbol}.`
                            : data.message,
                })
            }
        ...
        <div className="min-h-screen bg-gray-100 py-10 px-4">
            <div className="max-w-md mx-auto">
                ...
                <div className="bg-white rounded-2xl shadow-md p-8">
                    ...
                    {/* If user has no crypto */}
                    {ownedQty <= 0 ? (
                        <div className="text-center text-gray-400 py-6">
                            <p>You don't own any {crypto.name}.</p>
                            <button
                                onClick={() =>
                                    navigate(`/user/buy/${crypto.id}`)
                                }
                                className="mt-4 text-blue-500 hover:underline text-sm"
                            >
                                Buy {crypto.symbol} instead
                        ...
                        <>
                            ...
                            {quantity > 0 && (
                                <div
                                    className={`flex justify-between items-center mb-6 p-3 rounded-xl text-sm ${
                                        insufficientCrypto
                                            ? 'bg-red-50'
                                            : 'bg-green-50'
                                    }`}
                                >
                                    ...
                                    <span
                                        className={`font-bold text-lg ${insufficientCrypto ? 'text-red-500' : 'text-green-500'}`}
                                    >
                                        {fmt(totalReceived)} €
                                    </span>
                                </div>
