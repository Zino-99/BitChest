...
export default function AdminMarket() {
    // State for crypto list, selected crypto, historical data, loading/error states
    const [cryptos, setCryptos] = useState([])
    const [selected, setSelected] = useState(null)
    const [history, setHistory] = useState([])
    const [loadingChart, setLoadingChart] = useState(false)
    ...
    // Fetch list of cryptocurrencies on component mount
    useEffect(() => {
        fetch('https://127.0.0.1:8000/api/market', {
            headers: { 'Content-Type': 'application/json' },
        })
    ...
    // Fetch historical price data when selected crypto changes
    useEffect(() => {
        if (!selected) return
        setLoadingChart(true) // show chart loading spinner
        setHistory([]) // reset history
        fetch(`https://127.0.0.1:8000/api/market/${selected.id}/history`)
            .then((res) => res.json())
            .then((data) => setHistory(data))
            .finally(() => setLoadingChart(false)) // hide chart loading spinner
    }, [selected])

    // Display loading spinner while fetching market data
    if (loading)
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="flex flex-col items-center gap-3">
    ...
    // Determine color and positive/negative change for selected crypto
        ...
        <div className="min-h-screen bg-gray-50 p-6">
            ...
            <div className="mb-6">
                <h1 className="text-2xl font-bold text-gray-800">
                    Market Overview <span className="text-blue-500">📈</span>
                </h1>
            ...
            <div className="flex gap-6 h-[calc(100vh-160px)]">
                ...
                <div className="w-72 flex-shrink-0 flex flex-col gap-2 overflow-y-auto pr-1">
                    {cryptos.map((crypto) => {
                        ...
                        return (
                            <button
                                key={crypto.id}
                                onClick={() => setSelected(crypto)} // select crypto on click
                                className={`w-full text-left rounded-2xl px-4 py-3 transition-all ring-1 ${
                                    isSelected
                                        ? 'bg-white ring-blue-300 shadow-md'
                                        : 'bg-white ring-gray-100 hover:ring-gray-200 shadow-sm'
                ...
                {selected && (
                    <div className="flex-1 bg-white rounded-2xl shadow-sm ring-1 ring-gray-100 p-6 flex flex-col">
                        {/* Crypto header — display symbol and name */}
                        <div className="flex items-center gap-4 mb-6">
                            <div
                                className="w-12 h-12 rounded-2xl flex items-center justify-center text-white font-bold shadow"
                                style={{
                        ...
                        <div className="flex items-end gap-4 mb-6">
                            ...
                            <p
                                className={`text-lg font-semibold flex items-center gap-1 mb-1 ${
                                    positive
                                        ? 'text-emerald-500'
                                        : 'text-red-500'
                                }`}
                            ...
                            <span className="mb-1 ml-1 text-xs text-gray-300 uppercase tracking-widest">
                                vs previous
                            </span>
                        </div>
                        ...
                        <div className="flex-1">
                            {loadingChart ? (
                                // Show spinner while chart data is loading
                                <div className="h-full flex items-center justify-center">
                                    <div className="w-6 h-6 border-2 border-blue-300 border-t-transparent rounded-full animate-spin" />
                                </div>
                            ) : (
