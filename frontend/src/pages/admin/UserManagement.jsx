...
export default function UserManagement() {
    ...
    const [loading, setLoading] = useState(true) // loading state for table/cards
    const [showCreate, setShowCreate] = useState(false) // toggle create modal
    const [editUser, setEditUser] = useState(null) // selected user to edit
    const [deleteUser, setDeleteUser] = useState(null) // selected user to delete
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            showMsg('error', 'Unable to load users') // show error toast
        } finally {
    ...
    // Helper: show a temporary toast message
    const showMsg = (type, text) => {
        setMessage({ type, text })
        setTimeout(() => setMessage(null), 3000) // remove message after 3s
    }
    ...
    // Helper: return CSS class for role badge
    const roleColor = (role) =>
        role === 'admin'
            ? 'bg-amber-100 text-amber-700 border border-amber-200'
            : 'bg-blue-50 text-blue-600 border border-blue-100'
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {users.map((u) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4">
                                            <span
                                                className={`px-2.5 py-1 rounded-full text-xs font-medium ${roleColor(u.role)}`}
                                            >
                                                {u.role === 'admin'
                                                    ? 'Admin'
                                                    : 'User'}
